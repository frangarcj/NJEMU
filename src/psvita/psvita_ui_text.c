/******************************************************************************
	...
	User Interface Text Management
...
#define LANGUAGE_SIMPL_CHINESE 0
#define LANGUAGE_TRAD_CHINESE 1
#define LANGUAGE_JAPANESE 2
#define LANGUAGE_SPANISH 3
#define LANGUAGE_ENGLISH 4

static int configGetLanguage() {
	return LANGUAGE_ENGLISH;
}

static void *psvita_init(void)
{
	psvita_ui_text_t *psvita = (psvita_ui_text_t*)calloc(1, sizeof(psvita_ui_text_t));
	int i;

    switch (configGetLanguage())
	{
	case LANGUAGE_SIMPL_CHINESE:
		psvita->lang = LANG_CHINESE_SIMPLIFIED;
		for (i = 0; i < UI_TEXT_MAX; i++)
			psvita->ui_text[i] = text_CHINESE_SIMPLIFIED[i];
		break;

	case LANGUAGE_TRAD_CHINESE:
		psvita->lang = LANG_CHINESE_TRADITIONAL;
		for (i = 0; i < UI_TEXT_MAX; i++)
			psvita->ui_text[i] = text_CHINESE_TRADITIONAL[i];
		break;

	case LANGUAGE_JAPANESE:
		psvita->lang = LANG_JAPANESE;
		for (i = 0; i < UI_TEXT_MAX; i++)
			psvita->ui_text[i] = text_JAPANESE[i];
		break;

	case LANGUAGE_SPANISH:
		psvita->lang = LANG_SPANISH;
		for (i = 0; i < UI_TEXT_MAX; i++)
			psvita->ui_text[i] = text_SPANISH[i];
		break;

	default:
		psvita->lang = LANG_ENGLISH;
		for (i = 0; i < UI_TEXT_MAX; i++)
			psvita->ui_text[i] = text_ENGLISH[i];
		break;
	}
...
{
	psvita_ui_text_t *psvita = (psvita_ui_text_t*)data;
	free(psvita);
}

static int32_t psvita_getLanguage(void *data)
{
	psvita_ui_text_t *psvita = (psvita_ui_text_t*)data;
	return psvita->lang;
}

static const char *psvita_getText(void *data, int32_t id)
{
	psvita_ui_text_t *psvita = (psvita_ui_text_t*)data;
	return psvita->ui_text[id];
}


ui_text_driver_t ui_text_psvita = {
	"psvita",
	psvita_init,
	psvita_free,
	psvita_getLanguage,
